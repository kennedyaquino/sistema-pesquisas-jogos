...
const BASE_URL = 'http://192.168.0.114:8080';
...
const CreateRecord = () => {
    ...
    const [Platform, setPlataform] = useState<GamePlatform>();
    const [selectedGame, setSelectedGame] = useState('');
    const [allGames, setAllGames] = useState<Game[]>([]);
    const [filteredGames, setFilteredGames] = useState<Game[]>([]);

    const handleChangePlatform = (selectedPlatform: GamePlatform) => {
        setPlataform(selectedPlatform);
    ...
    const handleSubmit = () => {
       ...
       axios.post(`${BASE_URL}/records`, payLoad)
        .then(() => {
            ...
            setAge('');
            setSelectedGame('');
            setPlataform(undefined);
        })
        .catch(() => Alert.alert('Erro ao cadastrar os dados!'))
    }

    // Quando o componente index é iniciado esse metodo é executado
    useEffect(() => {
